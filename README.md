...
This module protects select apps' data from being wiped out on a regular TWRP factory reset. Thus, greatly reducing the amount of time required for setting up a new system.
...
How does this data protection thing actually work? At a glance, adk moves select apps' data to /data/media/adk/appdata. Next, it bind-mounts moved folders to their original locations. Thus, apps are fooled into "believing" their data is still in /data/data. TWRP doesn't wipe /data/media when performing a regular factory reset. Of course, this is an overly simplified explanation. Refer to the source code for extensive info.
